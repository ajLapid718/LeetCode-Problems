...
def reverse_list(head)
  ...
  until current_node == nil # This line traverses the entire singly-linked list; this involves reversing the nodes through each iteration in an as-we-go manner.
      ...
      current_node.next = previous_node # Set the actual next node to point toward the previous node. This is where the bulk of the reversal process happens.
...
[x] we need the actual following node (2) to be set with a value of the previous_node (nil); this is necessary because everything in a linked list points toward something ahead of it AND does not point to anything prior to it (unless it is a doubly-linked list)...so this redirects that arrow so-to-speak
